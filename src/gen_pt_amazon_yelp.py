...
def write_text(fn, data, skip_tag=True):
    with open(fn, "w") as fw:
        for (asin, tag, text) in data:
            if not skip_tag:
                fw.write("\n"+asin+" "+tag+"\n")
            fw.write(text+"\n")

def amazon(keep_probs, with_test, train_data, dev_data, test_data):
    laptop_tag = None
    asin2cat = {}
    ...
    print("laptop tag is named as", laptop_tag)
        ...
        for l in f:
            ...
            if asin in asin2cat:
                ...
                if len(text) > 0:
                    ...
                    if rnd < keep_probs:
                        if rnd < 0.0005:
                            dev_data.append((asin, asin2cat[asin], text))
                        else:
                            train_data.append((asin, asin2cat[asin], text))
                    else:
                        test_data.append((asin, asin2cat[asin], text))
    

def yelp(keep_probs, with_test, train_data, dev_data, test_data):
    ...
    print("restaurant tag is named as", "Restaurants")
...
def main(keep_probs = 0.8, with_test = True):
    ...
    yelp(keep_probs, with_test, train_data, dev_data, test_data)
    amazon(keep_probs, with_test, train_data, dev_data, test_data)
    ...
    write_text("data/pt/domain_train.txt", train_data, skip_tag=False)
    write_text("data/pt/domain_dev.txt", dev_data, skip_tag=False)
    if with_test:
        write_text("data/pt/domain_test.txt", test_data, skip_tag=False)
